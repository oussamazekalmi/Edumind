...
@section('content')
            ...
            <div class="page-title">
                <div class="container">
                    <div class="card mb-0 border-0 rounded-1 shadow-sm bg-white position-relative">
                        <div class="ps-2">
                            <h5 class="m-3" style="font-weight:500 !important;">Modifier paiement pour {{ $eleve->nom }} {{ $eleve->prenom }}</h5>
                                    ...
                                    <div class="form-section">
                                        <div class="row ms-3" style="width:96%;">
                                            <div class="col-md-6 position-relative">
                                                ...
                                                <div class="custom-dropdown readonly-dropdown" id="customDropdown">
                                                    <div style="color: #B8B6B3 !important;" class="custom-dropdown-selected d-flex justify-content-between" id="customDropdownSelected">
                                                    ...
                                                    <input type="hidden" name="type" id="customDropdownValue" value="{{ ucfirst($paiement->type) }}" readonly="true">
                                                    @error('type')
                                            ...
                                            <div class="col-md-6 position-relative">
                                                ...
                                                <div class="custom-dropdown" id="frequenceDropdown">
                                                    ...
                                                    <input type="hidden" name="frequence_paiement" id="frequenceDropdownValue" value="{{ ucfirst($paiement->frequence_paiement) }}">
                                                </div>
                                    ...
                                    <div class="form-section">
                                        <div class="row ms-3" style="width:96%;">
                                            <div class="col-md-6 position-relative">
                                                ...
                                                <div class="custom-dropdown" id="modePaiementDropdown">
                                                    ...
                                                    <input type="hidden" name="mode_paiement" id="modePaiementDropdownValue" value="{{ ucfirst($paiement->mode_paiement) }}">
                                                </div>
                                                @error('mode_paiement')
                                                    <div class="text-danger" style="position:absolute; top: 110%; left:3%;">{{ $message }}</div>
                                                @enderror
                                            ...
                                            <div class="col-md-6 position-relative">
                                                ...
                                                <input type="number" id="montant" name="montant" value="{{ old('montant') ?? $paiement->montant + 0 }}"
                                        ...
                                        <div class="row ms-3 mt-5 mb-4" id="intervalSection" style="width:96%;">
                                            <div class="col-md-6 position-relative">
                                                ...
                                                <input type="month" id="interval_debut" name="interval_debut" value="{{ old('interval_debut', \Carbon\Carbon::parse($paiement->interval_debut)->format('Y-m')) }}"
                                                    class="custom form-control rounded-0 border-0 border-bottom mt-2">
                                            ...
                                            <div class="col-md-6 position-relative">
                                                ...
                                                <input type="month" id="interval_fin" name="interval_fin" value="{{ old('interval_fin', \Carbon\Carbon::parse($paiement->interval_fin)->format('Y-m')) }}"
                                                    class="custom form-control rounded-0 border-0 border-bottom mt-2">
                                                @error('interval_fin')
                                                    <div class="text-danger" style="position:absolute; top: 110%; left:3%;">{{ $message }}</div>
                                                @enderror
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            const restantScolarite = {{ $restant_scolarite ?? 0 }};
            const restantTransport = {{ $restant_transport ?? 0 }};
            ...
            const mensualiteScolarite = baseScolarite - (baseScolarite * discount / 100);
            const mensualiteTransport = baseTransport - (baseTransport * transport_discount / 100);
            ...
            function updateMontant() {
                ...
                if (type === 'inscription') {
                    ...
                    montantInput.max = restantInscription;

                    montantInput.addEventListener("input", () => {
                        const val = parseFloat(montantInput.value);
                        if (val > restantInscription) {
                            montantInput.value = restantInscription;
                        }
                    });

                }
                else if ((type === 'scolaire' || type === 'transport') && debut && fin) {
                    const mois = calculateMonths(debut, fin);
                    let montantCalculé = 0;
                    let max = 0;

                    if (type === 'scolaire') {
                        montantCalculé = mois * mensualiteScolarite;
                        max = restantScolarite;
                    } else if (type === 'transport') {
                        montantCalculé = mois * mensualiteTransport;
                        max = restantTransport;
                    }

                    const finalMontant = Math.min(montantCalculé, max);
                    montantInput.readOnly = true;
                    montantInput.value = finalMontant.toFixed(2);
                    montantInput.max = finalMontant;
            ...
            typeInput.addEventListener("change", updateMontant);
            intervalDebut.addEventListener("change", updateMontant);
            intervalFin.addEventListener("change", updateMontant);
        });
    </script>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function setInitialDropdownValue(dropdownId, value) {
                const dropdown = document.getElementById(dropdownId);
                if (!dropdown) return;
                ...
                options.forEach(option => {
                    if (option.getAttribute('data-value') === value.toLowerCase()) {
                        selected.textContent = option.textContent.trim();
                    }
                });
            }

            setInitialDropdownValue('customDropdown', typeInput.value);
            setInitialDropdownValue('frequenceDropdown', document.getElementById('frequenceDropdownValue').value);
            setInitialDropdownValue('modePaiementDropdown', document.getElementById('modePaiementDropdownValue').value);
