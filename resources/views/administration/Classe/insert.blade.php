        ...
        <div class="edit-profile">
            <div class="row d-flex justify-content-center">
                <div class="col-xl-12 col-lg-12">
                    <form class="card rounded-0 shadow-sm" action="{{ route('classes.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="card-body">
                            <div class="row">
                                    ...
                                    <div>
                                        ...
                                        <div class="custom-dropdown text-secondary mb-2" id="niveauDropdown">
                                            <div class="custom-dropdown-selected d-flex justify-content-between align-items-center" id="niveauDropdownSelected">
                                                <span id="niveauDropdownSelectedText" class="{{ old('niveau') ? '' : 'text-muted' }}">
                                                    {{ old('niveau') ? ($niveaux->firstWhere('id', old('niveau'))?->nom ?? 'Choisir un niveau') : 'Choisir un niveau' }}
                                                </span>
                                            ...
                                            <input type="hidden" name="niveau" id="niveauDropdownValue" value="{{ old('niveau') }}">
                                        </div>
                                        @error('niveau')
                                ...
                                <div class="col-md-4">
                                    <div class="my-2">
                                        <label class="form-label f-w-500">Nom de la classe</label>
                                        <input class="form-control forms-administartion rounded-0 mb-2 f-w-500 text-secondary" type="text" name="classe" value="{{ old('classe') }}" autocomplete="off">
                                        @error('classe')
                                            <small class="text-danger ms-1">{{ $message }}</small>
                                        @enderror
                                    </div>
