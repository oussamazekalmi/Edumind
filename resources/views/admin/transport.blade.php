          ...
          <div class="col-md-12">
            <div class="card rounded-0 shadow-sm">
                <div class="card-header pt-4 position-relative">
                    ...
                    <a class="rounded-0 text-dark py-2 shadow-sm" href="{{ route('inscription.frais') }}" style="position:absolute; top:0; left:40%; background-color: #FBFBFA; padding: 12px 18px;">
                        <i style="color:#B8B6B3;" class="fas fa-user-plus me-3"></i> Frais Inscription
                    </a>
                    <a class="rounded-0 text-dark py-2 shadow-sm" href="{{ route('scolaire.frais') }}" style="position:absolute; top:0; right:0; background-color: #FBFBFA; padding: 12px 18px;">
                ...
                <div class="card-body">
                    <div class="row  mt-5 mb-3 position-relative">
                        <div class="col-md-4 mx-5">
                            <form action="{{ route('transport.frais') }}" method="GET">
                                @csrf
                                <div class="row">
                        ...
                        @if(request()->has('mois') || request()->has('day'))
                            <a href="{{ route('transport.frais') }}" style="position:absolute; top:0%; right:6%;" class="btn col-md-1 py-3 rounded-5">
                                        ...
                                        <td>
                                            @php
                                                ...
                                                $formattedDate = $paymentDate->isYesterday()
                                                    ? '<span style="color:#6495ED">Hier</span>'
                                                    : ($paymentDate->isToday()
                                                        ? '<span style="color:#6495ED;">Aujourd\'hui</span>'
                                                        : '<span style="color:#6495ED;">Le </span>' . $paymentDate->isoFormat('D MMMM Y'));
                                            @endphp
                                            {!! $formattedDate !!}
                                        </td>
                    ...
                    <input  type="text" class="form-control text-center fw-bold shadow-sm" style="background-color:#F8F8F7; color: lightgray; border:none; font-weight:500 !important;" value="{{

                        isset($dailyTotal) && $dailyTotal !== null
                        ? number_format($dailyTotal->total_montant, 2)
                        : (isset($monthlyTotals) && $monthlyTotals->first() !== null
                        ? number_format($monthlyTotals->first()->total_montant, 2)
                        : (isset($totalGeneral)
